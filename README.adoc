...
Here you create a `application.yml` file for container.
...
- We disable batch functionality in Boot core to use a YARN specific.
- We could optionally add Hadoop configuration for HDFS.
...
Here you create a `application.yml` file for appmaster.
...
Here you create a `application.yml` file for client.
...
- Here we simply defined all needed files for the application needed
  for submission.
